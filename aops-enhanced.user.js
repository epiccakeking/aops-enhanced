...
let settings_ui = {
	...
	button: (label, newFunction) => (name, value, settings_manager) => {
...
function show_enhanced_configurator() {
	UI_ELEMENTS = {
		...
		forum_colors: settings_ui.button("Change Forum Colors", openColorModal),
	}
...
function openColorModal() {
	...
	for (let i = 0; i < mainColor.length; i++) {
		...
		tempDiv.querySelector(".forum-id").setAttribute("value", forumId[i])
		innerModal.innerHTML = innerModal.innerHTML + tempDiv.innerHTML
	}
	innerModal.innerHTML = innerModal.innerHTML + sideButtons
	innerModal.querySelector(".delete-color").onclick = deleteColor
	innerModal.querySelector(".add-color").onclick = addColor
	innerModal.querySelector(".save-color").onclick = setColorToStorage
	alert(innerModal)
}
function deleteColor() {
	var tempDiv = innerModal.querySelectorAll(".picker-group")
...
function addColor() {
	...
	innerModal.insertBefore(
		tempDiv,
		innerModal.childNodes[innerModal.childNodes.length - 5]
	)
}
function setColorToStorage() {
	var tempElement = innerModal.querySelectorAll(".picker-group")
	for (let i = 0; i < tempElement.length; i++) {
		...
		if (tempElement[i].querySelector(".forum-id").value != "") {
			forumId[i] = tempElement[i].querySelector(".forum-id").value
		} else {
			forumId[i] = 0
		}
	}
	var temp = ""
	for (let i = 0; i < mainColor.length; i++) {
		temp = temp + mainColor[i] + "|" + secondColor[i] + "|" + forumId[i] + " "
	}
	localStorage.setItem("color", temp)
}

myInterval = setInterval(setFunction, 10)
//const mainColor = "#e38a1e"
//const secondColor = "#ffebd1"
function setFunction() {
	if (typeof AoPS.Community.Views !== "undefined") {
		clearInterval(myInterval)
		AoPS.Community.Views.CategoryCell.prototype.applyColor = (function () {
			var cached_function =
				AoPS.Community.Views.CategoryCell.prototype.applyColor

			return function () {
				for (let i = 0; i < forumId.length; i++) {
					if (this.model.attributes.category_id == forumId[i]) {
						this.model.attributes.main_color = mainColor[i]
						this.model.attributes.secondary_color = secondColor[i]
					}
				}
		...
		AoPS.Community.Views.TopicCell.prototype.setColor = (function () {
			var cached_function = AoPS.Community.Views.TopicCell.prototype.setColor

			return function () {
				for (let i = 0; i < forumId.length; i++) {
					if (this.model.attributes.category_id == forumId[i]) {
						this.model.attributes.category_main_color = mainColor[i]
						this.model.attributes.category_secondary_color = secondColor[i]
					}
				}
				var result = cached_function.apply(this, arguments) // use .apply() to call it
		...
		AoPS.Community.Views.CategoryTagFilter.prototype.setFocusTag =
			(function () {
				var cached_function =
					AoPS.Community.Views.CategoryTagFilter.prototype.setFocusTag
				...
				return function () {
					for (let i = 0; i < forumId.length; i++) {
						if (
							this.focus_tag != null &&
							this.model.attributes.category_id == forumId[i]
						) {
							this.focus_tag.item_main_color = mainColor[i]
							this.focus_tag.item_secondary_color = secondColor[i]
						}
					}
					var result = cached_function.apply(this, arguments) // use .apply() to call it
		...
		AoPS.Community.Views.Itembox.prototype.constructItem = (function () {
			var cached_function = AoPS.Community.Views.Itembox.prototype.constructItem

			return function () {
				for (let i = 0; i < forumId.length; i++) {
					if (this.model.attributes.category_id == forumId[i]) {
						for (let j = 0; j < this.model.attributes.items.length; j++) {
							this.model.attributes.items[j].item_main_color = mainColor[i]
							this.model.attributes.items[j].item_secondary_color =
								secondColor[i]
						}
					}
				}
